...
geom_point(aes(x = "length", 
           y = "diameter", 
           color = "sex",
           size = "shell_weight"))+   scale_size_discrete() +
...
model
```
When $diameter$ increased by $1$, $height$ will increase by $0.351376$. The intercept indicates that whehn diameter is zero, height will be -0.003803. In this context, the intercept is not relevant since it is not possible for there to be zero diameter. 
------------------------------------------------------------------------
...
```{R}
plot(df$diameter, df$height) + abline(model, color = "red") #FIX
```
