...
```{R}
df %>%
  mutate(num_rings = case_when(rings < 10 ~ "low", rings >20 ~ "high", rings >= 10 | rings <= 20 ~ "med")) %>%
  group_by(num_rings,sex) %>%
  summarize(avg_weight = sum((whole_weight + shucked_weight + viscera_weight + shell_weight)/4)) %>%
    ggplot(df,
           mapping = aes(x = num_rings, y = sex, color = avg_weight)) + 
    geom_tile()
```
...
model
```
When $diameter$ increased by $1$, $height$ will increase by $0.351376$. The intercept indicates that when diameter is zero, height will be -0.003803. In this context, the intercept is not relevant since it is not possible for there to be zero diameter. 
------------------------------------------------------------------------
...
#for (value in new_diameters){
 # expected <-append(0.003803 + 0.351376 *value)
#}
